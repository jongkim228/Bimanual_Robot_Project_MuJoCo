...
void BottlePoseController::starting(const ros::Time&){auto s=state_handle_->getRobotState();hold_pose_=s.O_T_EE_d;std::ostringstream os;
  for(int i=0;i<16;++i) os << s.O_T_EE_d[i] << " ";
  ROS_WARN_STREAM("O_T_EE_d = " << os.str());start_pose_=hold_pose_;active_=false;start_req_=false;stop_req_=false;pose_handle_->setCommand(hold_pose_);}
